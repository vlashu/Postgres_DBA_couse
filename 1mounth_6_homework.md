...
### Выводы:
За время выполнения ДЗ было проведено более 300 запусков теста.

Отличия выключенного autovacuum от агрессивных настроект практически не заметны.
 
В некоторых тестах было видно влияние, однако:
 1) Тест не воспроизводился (повторный запуск не гарантировал стабильные показания)
 2) При запуске теста в более длительном режиме эффект проявлялся, но позднее
 
Также замечено, что при смене на Balanced disc tps падает примероно на 100 секунде до значения 300tps и стабильно держится на этом значении.
